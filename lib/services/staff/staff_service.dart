...
class StaffService {
  ...
  Stream<QuerySnapshot> getStaffStream({
    ...
    if (selectedBranchId != null && selectedBranchId.isNotEmpty) {
      query = query.where('branchIds', arrayContains: selectedBranchId).orderBy('name');
    } else if (branchIds.isNotEmpty && branchIds.length <= 10) {
      query = query.where('branchIds', arrayContainsAny: branchIds.take(10).toList()).orderBy('name');
    } else {
  ...
  Future<void> updateStaff(String staffId, Map<String, dynamic> data, String updatedBy) async {
  ...
  Stream<QuerySnapshot> getShiftsStream({String? selectedBranchId, List<String>? branchIds}) {
  ...
  Stream<int> getClockedInTodayCount({String? selectedBranchId, List<String>? branchIds}) {
    final today = _todayString();
    Query query = _db
        .collection('attendance')
        .where('date', isEqualTo: today)
  ...
  Stream<int> getTodayShiftCount({String? selectedBranchId, List<String>? branchIds}) {
